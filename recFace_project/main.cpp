...
string haar_face_datapath = "/home/polya/mine/opencv_for_c++/recFace_project/haarcascade_frontalface_alt_tree.xml";
int main(int argc, char** argv) {
	...
	while (capture.read(frame)) {
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			putText(frame, format("i'm (%d) %s", label, (label == 41 ? "Yaomin" : "Unknow")), faces[i].tl(), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 0, 255), 2, 8);
		}//putText(frame, format("i'm %s", (label == 41 ? "Yaomin" : "Unknow")), faces[i].tl(), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 0, 255), 2, 8);
